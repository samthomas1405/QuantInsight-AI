...
import { Search, TrendingUp, X, RefreshCw, Copy, Download, FileText, Clock, BarChart3, Shield, Target, MessageSquare } from 'lucide-react';
import { useTheme } from '../contexts/ThemeContext';
import { getReports } from '../api/news';
import { fetchLiveFollowedMarketData } from '../api/liveMarket';
import { searchStocks } from '../api/stock';
import CompanyLogo from './CompanyLogo';

const MultiAgentPredictor = () => {
  ...
  // Load available tickers from followed stocks
  useEffect(() => {
  ...
  const runAnalysis = async () => {
    ...
    setError(null);
    setReports({});
    setLoadingTickers({}); // Clear all loading states first
    ...
    try {
      // Only set loading state for tickers we're actually analyzing
      const tickersToAnalyze = selectedTickers.slice(); // Create a copy
      const loadingState = {};
      ...
      // API call with selected tickers
      const response = await getReports(token, tickersToAnalyze);
      
      if (response && response.reports) {
        // Only process tickers that were returned by the API
        const tickerList = Object.keys(response.reports);
        ...
        // Stream results one at a time for better UX
  ...
  const handleRerun = async (ticker) => {
    ...
    const token = localStorage.getItem('token');
    try {
      const response = await getReports(token, [ticker]);
    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="mb-6">
        ...
        <p className={`text-sm mt-1 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
          AI-powered technical, fundamental, and sentiment analysis
        </p>
      </div>
      ...
      {(lastRunTime || Object.keys(reports).length > 0) && (
        ...
        >
          <div>
            <p className={`text-xs ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>Tickers selected</p>
      ...
      {selectedTickers.length === 0 && Object.keys(reports).length === 0 && (
        ...
        >
          ...
          <p className="text-sm mt-1">Search for any stock or choose from your followed list</p>
        </motion.div>
      )}
      ...
      <AnimatePresence>
        {selectedTickers.map(ticker => {
          ...
          // Only show ticker card if we have a report or it's loading
...
// Analysis Report Component with Professional Copy
const AnalysisReport = ({ ticker, data, isDark }) => {
  const sections = [
    ...
    },
    {
      ...
      icon: TrendingUp,
      formatter: (content) => {
        // Extract technical levels and key observations
        const cleanContent = content.replace(/market.*?analysis.*?:/gi, '').trim();
        
        // Look for price levels and technical indicators
        const priceMatch = cleanContent.match(/\$?\d+\.?\d*/g);
        const price = priceMatch ? priceMatch[0] : '';
        
        // Generate concise technical commentary
    ...
    },
    {
      ...
      formatter: (content) => {
        ...
        if (sentences.length > 0) {
          // Take first relevant sentence about fundamentals
    ...
    },
    {
      ...
      formatter: (content) => {
        ...
        // Fallback professional strategy points
        return [
  ...
  return (
    <div className="space-y-5">
      {sections.map(({ key, title, icon: Icon, formatter }) => {
...
export default MultiAgentPredictor;